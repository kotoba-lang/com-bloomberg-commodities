(ns bloomberg_commodities.main
  "Kotodama WASM entrypoint for the Bloomberg_commodities clean-room actor (L4) — Clojure port.
  ...
  facts (`bloomberg_commodities.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bloomberg_commodities")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bloomberg_commodities.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bloomberg_commodities-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bloomberg_commodities-compat/ready)
