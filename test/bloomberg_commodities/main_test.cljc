(ns bloomberg_commodities.main-test
  "Contract + behavioral test for the bloomberg_commodities-compat L4 actor (cljc port).
  ...
  (:require [clojure.test :refer [deftest is testing run-tests]]
            ...
            [bloomberg_commodities.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "bloomberg_commodities-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'bloomberg_commodities.main-test)]
